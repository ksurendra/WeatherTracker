Feature: Add a measurement
  ...
  @new
  Scenario: Add a measurement with valid (numeric) values but different headers
    # POST /measurements
    When I submit a new measurement as follows:
      | timestamp                  | temperatureX | dewPointY | precipitationZ |
      | "2015-09-01T16:00:00.000Z" | 29.1         | 16.0      | 1              |
    Then the response has a status code of 201
    And the Location header has the path "/measurements/2015-09-01T16:00:00.000Z"
  ...
  @new
  Scenario: Cannot add a measurement with invalid values (dew point)
    # POST /measurements
    When I submit a new measurement as follows:
      | timestamp                  | temperature    | dewPoint        | precipitation |
      | "2015-09-01T16:00:00.000Z" | 23.1           | "not a number"  | 0             |
    Then the response has a status code of 400

  @new
  Scenario: Cannot add a measurement with invalid values (precipitation)
    # POST /measurements
    When I submit a new measurement as follows:
      | timestamp                  | temperature    | dewPoint  | precipitation |
      | "2015-09-01T16:00:00.000Z" | 23.1           | 13.2      | "not a numbe" |
    Then the response has a status code of 400
